...
head(ramus, 3) %>% knitr::kable("simple", caption = "The first 3 samples")
tail(ramus, 3) %>% knitr::kable("simple", caption = "The last 3 samples")
```

##

```{r class.source="bg-danger", class.output="bg-warning"}
ramus_mean <- map(ramus[2:5], mean) 
ramus_mean %>% 
  as.data.frame() %>% 
  round(4) %>%
  knitr::kable("simple", caption = "Mean")

ramus_sd <- map(ramus[2:5], sd) 
ramus_sd %>% 
  as.data.frame() %>% 
  round(4) %>%
  knitr::kable("simple", caption = "Standard deviation")

ramus_se <- map(ramus[2:5], ~ sd(.x)/sqrt(length(.x))) 
ramus_se %>%
  as.data.frame() %>%
  round(4) %>%
